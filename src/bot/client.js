...
client.on('interactionCreate', async interaction => {
  ...
  // Handle Abort Cancel button
  if (interaction.isButton() && interaction.customId === 'cancel_abort') {
    try {
      const language = await getInteractionLanguage(interaction);
      if (interaction.message.embeds && interaction.message.embeds.length > 0) {
        const { renderMyStuff } = require('./commands/mystuff');
        return renderMyStuff(interaction, true, null);
      } else {
        return interaction.update({
          content: translate(language, 'cancelAbortedMsg'),
          components: []
        });
      }
    } catch (err) {
      console.error('[client] cancel abort error:', err);
      return;
    }
  }

  // Intercept Initial Cancellation Buttons to Show Confirmation Prompt
  const isCancelAction = interaction.isButton() && (
    interaction.customId === 'unreserve_me' ||
    interaction.customId.startsWith('c_p_') ||
    interaction.customId.startsWith('c_i_') ||
    interaction.customId.startsWith('c_b_')
  );
  ...
  if (isCancelAction) {
    try {
      ...
      const confirmRow = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId(`confirm_cancel_${interaction.customId}`)
          .setLabel(translate(language, 'confirmCancelBtn'))
          .setStyle(ButtonStyle.Danger),
        new ButtonBuilder()
          .setCustomId('cancel_abort')
          .setLabel(translate(language, 'cancelAbortBtn'))
          .setStyle(ButtonStyle.Secondary)
      );

      return interaction.update({
        content: translate(language, 'confirmCancelPrompt'),
        components: [confirmRow]
      });
    } catch (err) {
      const language = await getInteractionLanguage(interaction);
      console.error('[client] confirm prompt error:', err);
  ...
  // Handle Confirmed Cancellation Buttons
  if (interaction.isButton() && interaction.customId.startsWith('confirm_cancel_')) {
    const realCustomId = interaction.customId.replace('confirm_cancel_', '');

    if (realCustomId === 'unreserve_me') {
      try {
        const language = await getInteractionLanguage(interaction);
        const db = require('../db/queries');
    ...
    if (realCustomId.startsWith('c_p_') || realCustomId.startsWith('c_i_') || realCustomId.startsWith('c_b_')) {
      try {
        const language = await getInteractionLanguage(interaction);
        const db = require('../db/queries');
        const { updateLiveBoard } = require('./liveboard');
        ...
        if (realCustomId.startsWith('c_p_')) {
          const pageId = realCustomId.replace('c_p_', '');
          console.log(`[Cancel] User ${discordUsername} confirmed to cancel Page ${pageId}`);
          await db.deletePageReservationsForUser(currentRound.id, pageId, interaction.user.id);
        } else if (realCustomId.startsWith('c_b_')) {
          const itemIds = realCustomId.replace('c_b_', '').split('_');
          console.log(`[Cancel] User ${discordUsername} confirmed to cancel bulk items:`, itemIds);
          for (const id of itemIds) {
            if (id) await db.deleteSingleReservation(currentRound.id, id, interaction.user.id);
          }
        } else {
          const itemId = realCustomId.replace('c_i_', '');
          console.log(`[Cancel] User ${discordUsername} confirmed to cancel Item ID: ${itemId}`);
